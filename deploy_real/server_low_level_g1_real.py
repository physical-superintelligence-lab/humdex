...
class RealTimePolicyController(object):
    ...
    def __init__(self, 
                 ...
                 smooth_body=0.0,
                 safety_rate_limit=True,
                 safety_rate_limit_scope="all",
                 max_dof_delta_per_step=0.05,
        ...
        # Safety rate limiter (prevents huge jumps on bugs)
        self.safety_rate_limit = bool(safety_rate_limit)
        self.safety_rate_limit_scope = str(safety_rate_limit_scope)
        self.max_dof_delta_per_step = float(max_dof_delta_per_step)
        self.max_dof_delta_print_every = int(max(1, max_dof_delta_print_every))
        self._delta_clip_counter = 0
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Safety: rate limit per-step joint delta via interpolation
                # If any joint delta is too large, shrink the whole delta vector proportionally:
                #   target' = current + alpha*(target-current), alpha in (0,1]
                if self.safety_rate_limit:
                    cur = np.asarray(dof_pos, dtype=np.float32).reshape(-1)
                    tgt = np.asarray(target_dof_pos, dtype=np.float32).reshape(-1)
                    if cur.shape == tgt.shape and cur.size > 0:
                        delta = tgt - cur

                        # Choose which joints to limit
                        if self.safety_rate_limit_scope == "arms":
                            # g1.yaml ordering: [legs 0-11][waist 12-14][left arm 15-21][right arm 22-28]
                            idx = np.arange(15, min(29, cur.size), dtype=np.int32)
                        else:
                            idx = np.arange(cur.size, dtype=np.int32)

                        if idx.size == 0:
                            max_abs = 0.0
                        else:
                            max_abs = float(np.max(np.abs(delta[idx])))
                        if max_abs > self.max_dof_delta_per_step:
                            # alpha = float(self.max_dof_delta_per_step / max_abs) / 10.0
                            # print(alpha)
                            # alpha = 0.1
                            alpha = 0.1
                            tgt2 = tgt.copy()
                            tgt2[idx] = cur[idx] + alpha * delta[idx]
                            target_dof_pos = tgt2.astype(np.float32)
...
def main():
    ...
    parser.add_argument('--safety_rate_limit', action='store_true',
                        help='Enable safety rate limiter on target_dof_pos (prevents large jumps)')
    parser.add_argument('--safety_rate_limit_scope', choices=["all", "arms"], default="all",
                        help='Which joints to rate limit when safety_rate_limit enabled (default: all; arms limits indices 15-28)')
    parser.add_argument('--max_dof_delta_per_step', type=float, default=0.05,
                        help='Max abs joint delta per control step (rad) when safety_rate_limit enabled')
    parser.add_argument('--max_dof_delta_print_every', type=int, default=200,
                        help='Print safety limiter logs every N clamps (default: 200)')
    ...
    print(f"  Safety rate limit: {args.safety_rate_limit}")
    ...
    controller = RealTimePolicyController(
        ...
        safety_rate_limit=args.safety_rate_limit,
        safety_rate_limit_scope=args.safety_rate_limit_scope,
        max_dof_delta_per_step=args.max_dof_delta_per_step,
        max_dof_delta_print_every=args.max_dof_delta_print_every,
    )

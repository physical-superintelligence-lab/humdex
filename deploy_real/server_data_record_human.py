...
def build_redis_key_candidates(channel: str, suffix: str, *, body_from_redis: bool = True) -> List[tuple[str, List[str]]]:
    base: List[tuple[str, List[str]]] = [
        ...
        ("action_hand_left", [f"action_hand_left_{suffix}"]),
        ("action_hand_right", [f"action_hand_right_{suffix}"]),
        ("action_neck", [f"action_neck_{suffix}"]) if body_from_redis else ("action_neck", []),
        ("t_action", ["t_action"]),
...
def main() -> int:
    ...
    print("=" * 70)
    print("VDMocap/Redis + RealSense Recorder (record-only)")
    print("=" * 70)

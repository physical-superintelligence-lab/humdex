...
class WujiHandRedisController:
    ...
    def __init__(
        ...
        policy_tag: str = "geort_filter_wuji",
        policy_epoch: int = -1,
            ...
            while self.running:
                ...
                if is_active and hand_data_dict is not None:
                    try:
                        ...
                        # 3. model-based or optimal-based
                        if self.use_model:
...
示例:
  ...
  python server_wuji_hand_redis.py --hand_side left --no_smooth
    ...
    parser.add_argument(
        "--use_model",
        action="store_true",
        help="使用 model-based（GeoRT）；默认 optimal-based（WujiHandRetargeter）",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        ...
        help="内置 checkpoint 别名：filter / filter_v2",
    )
    parser.add_argument(
        "--policy_tag",
        type=str,
        default="",
        help="自定义模型 tag；为空时使用 --checkpoint 对应值",
    )
    parser.add_argument("--policy_epoch", type=int, default=-1, help="模型 epoch，-1 表示 last")
    parser.add_argument("--use_fingertips5", action="store_true", help="model 模式下使用 5 指尖输入（默认开启）")
...
def main():
    ...
    print("=" * 60)
    print("Wuji Hand Controller via Redis (26D -> 21D MediaPipe)")
    print("=" * 60)
    ...
    print(f"Mode: {'model-based' if args.use_model else 'optimal-based'}")
    if args.use_model:

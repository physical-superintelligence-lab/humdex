...
class GeoRTTrainer:
    ...
    def train(self, human_data_path, **kwargs):
        ...
        save_every = kwargs.get("save_every", 10)  # checkpoint interval
        ...
        # --- NEW: record scaling for reproducibility ---
        # User-provided per-finger ratio: (human_len / robot_len), robot is the denominator.
        # To map human points into robot scale, we divide by this ratio (equivalently multiply by robot/human).
        ...
        export_config["human_point_scaling"] = {
            ...
            "note": "scaled_human = human / (human/robot) = human * (robot/human)",
        }
        # ---------------------------------------------
        ...
        human_points_raw = npz["fingertips_rel_wrist"]  # [T, 5, 3] (assumed meters)

        # --- NEW: apply per-finger scaling BEFORE transpose/dataset ---
        scale = np.asarray(WUJI_FINGER_TIP_SCALING, dtype=np.float32)  # [5]
        if human_points_raw.shape[1] != 5:
            raise ValueError(
                f"Expect fingertips_rel_wrist second dim = 5 fingers, got {human_points_raw.shape}"
            )

        # Use only xyz
        human_xyz = human_points_raw[:, :, :3].astype(np.float32)  # [T,5,3]

        # IMPORTANT: since scaling list is (human/robot) and robot is denominator, divide to map to robot scale
        # human_scaled[t, i, :] = human_xyz[t, i, :] / scale[i]
        human_xyz_scaled = human_xyz / scale[None, :, None]  # broadcast [T,5,3] / [1,5,1]

        human_points = np.transpose(human_xyz_scaled, (1, 0, 2)).astype(np.float32)  # [5, T, 3]
        # ------------------------------------------------------------
        ...
        for epoch in range(n_epoch):
            for batch_idx, batch in enumerate(point_dataloader):
                point = batch.cuda()  # [B, 5, 3] (already scaled)
                joint = ik_model(point)  # [B, DOF]
                embedded_point = fk_model(joint)  # [B, 5, 3]
